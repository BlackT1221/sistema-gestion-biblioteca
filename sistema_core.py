...
# sistema_core.py (al final del archivo)
from modelos import Libro, Usuario
from arboles import ArbolBST

class SistemaBibliotecaArbol:
    """
    Sistema de gestión de biblioteca usando árboles binarios de búsqueda (BST)
    para optimizar las operaciones de búsqueda e inserción.
    """
    def __init__(self):
        self.libros = ArbolBST()
        self.usuarios = ArbolBST()
        self.prestamos = {}  # {id_usuario: [isbn1, isbn2, ...]}

    def agregar_libro(self, libro: Libro):
        existente = self.libros.buscar(libro.isbn)
        if existente:
            print(f"El libro '{libro.titulo}' ya existe. No se duplica.")
        else:
            self.libros.insertar(libro.isbn, libro)
            print(f"Libro agregado: {libro.titulo}")

    def registrar_usuario(self, usuario: Usuario):
        if self.usuarios.buscar(usuario.id):
            print(f"El usuario {usuario.nombre} ya está registrado.")
        else:
            self.usuarios.insertar(usuario.id, usuario)
    ...
    def devolver_libro(self, id_usuario: str, isbn: str) -> bool:
        ...
        if usuario is None or libro is None:
            print("⚠️ Usuario o libro no encontrado.")
            return False
        if id_usuario not in self.prestamos or isbn not in self.prestamos[id_usuario]:
            print("⚠️ No se encontró un préstamo activo para este usuario/libro.")
            return False

        libro.estado = "disponible"
        usuario.libros_prestados.remove(libro.id)
        self.prestamos[id_usuario].remove(isbn)
        self.libros.insertar(isbn, libro)
        print(f"✅ Devolución registrada: {usuario.nombre} <- {libro.titulo}")
        return True

    def listar_libros(self):
        for _, libro in self.libros.recorrido_inorden():
            print(libro)

    def listar_usuarios(self):
        for _, usuario in self.usuarios.recorrido_inorden():
            print(usuario)
